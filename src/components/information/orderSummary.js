    ...
    render() {
        ...
        return (
            <div className={`${className} order-summary`}>
                ...
                <InfoLabel className='order-summary__subtotal' title={`"${amountStickers} stickers"`} value={`$${subtotal}`} />
                <InfoLabel className='order-summary__tax-shipping' title="Taxes & Shipping" value={`$${tax}`} />
                <InfoLabel className='order-summary__total info-title-green' title="Total" value={`$${subtotal + tax}`} />
            </div>
        )
    }
}
